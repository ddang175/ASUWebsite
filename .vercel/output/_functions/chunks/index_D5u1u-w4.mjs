...
const OFF_X = 2400;
const OFF_Y = 1600;
const FLY_IN_EASE = [0.16, 1, 0.3, 1];
const FLY_OFF_EASE = [0.7, 0, 0.84, 0];
    ...
    motion.div,
    {
      ...
      children: /* @__PURE__ */ jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsx("div", { className: "rounded-[2px] overflow-hidden bg-asu-beige", children: imageSrc ? /* @__PURE__ */ jsx(
              "img",
              {
                src: imageSrc,
                alt: imageAlt || title,
              ...
              "div",
              {
                ...
                style: {
                  background: gradient ?? "linear-gradient(135deg, #FCEEC9, #E8C66A)"
                },
                "aria-hidden": "true"
              }
            ) }),
            /* @__PURE__ */ jsxs("div", { className: "mt-1.5", children: [
              /* @__PURE__ */ jsx("p", { className: "font-marker text-[25px] md:text-[25px] leading-tight text-asu-dark truncate", children: title }),
              /* @__PURE__ */ jsx("p", { className: "font-marker text-[18px] md:text-[18px] leading-tight text-asu-dark opacity-50 mt-[3px] truncate", children: date })
            ] })
          ]
        }
      )
...
const THRESHOLDS = {
  width: {
    hideOuterAt: 1500,
    // inner (center) horizontal cards hide below this width
    hideMiddleAt: 1660,
    // middle left/right cards also hide below this
    hideAllAt: 1050
    // all cards hide below this width
  },
  height: {
    hideTopCenterAt: 1e3,
    // top-center cards hide below this height (before other top cards)
    hideOuterAt: 820,
    // reserved — assign verticalGroup: "middle-outer" to use
    hideAllAt: 550
    // all cards hide below this height
  }
};
const LARGE_SCREEN = {
...
const LOGO_HALF_WIDTH_DESKTOP = 250;
const ENTRY_OFFSETS = {
  left: { x: -2400, y: 0 },
  right: { x: 2400, y: 0 },
  top: { x: 0, y: -1600 },
...
function PolaroidGrid() {
  ...
  const largeProgress = width > 0 ? Math.min(
    1,
    Math.max(
      0,
      (width - LARGE_SCREEN.transitionStart) / (LARGE_SCREEN.transitionEnd - LARGE_SCREEN.transitionStart)
    )
  ) : 0;
        ...
        children: [
          ...
          POLAROIDS.map((card, i) => {
            ...
            if (largeProgress > 0) {
              ...
              const vFraction = horizontalGroup === "outer" ? 1 : horizontalGroup === "middle" ? 0.5 : 0;
              ...
              if (hDelta > 0.5) {
                ...
                } else if (flyOffDirection === "right" && card.position.right != null) {
...
const $$Hero = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${maybeRenderHead()}<section class="relative bg-asu-ivory flex flex-col overflow-x-clip min-[2000px]:pb-40 min-[1920px]:pb-15" style="min-height: calc(100vh - 4rem);"> <!--
  ...
  --> <div class="relative flex flex-col flex-1 w-full" style="max-width: 1920px; margin-inline: auto;"> <!-- 1. Polaroid collage (React island, hydrates on load) --> ${renderComponent($$result, "PolaroidGrid", PolaroidGrid, { "client:load": true, "client:component-hydration": "load", "client:component-path": "C:/Bench/ASUWebsite/src/components/home/PolaroidGrid", "client:component-export": "PolaroidGrid" })} <!-- 2. Headline + CTA
         ...
         overlaps them. The CTA link re-enables pointer events explicitly. --> <div class="flex flex-col items-center justify-center text-center px-8 pt-2 pb-8 flex-1 -translate-y-5 pointer-events-none"> <p class="font-ui text-xs font-bold tracking-[0.1em] uppercase text-asu-dark opacity-50 mb-3">
Asian Student Union
</p> <h1 class="font-display text-[3rem] leading-[3.5rem] text-asu-dark" style="letter-spacing: -0.02em;">
@ Iowa State University
</h1> <a href="/about" class="mt-8 inline-flex items-center px-8 py-3 bg-asu-red text-asu-cream font-ui font-semibold text-base rounded transition-colors duration-200 hover:bg-asu-red-hover pointer-events-auto">
About Us
</a> </div> </div> <!-- 3. Scroll cue — "NEXT EVENT" label + bouncing arrow --> <div class="flex flex-col items-center gap-1.5 pb-10 opacity-50"> <span class="font-ui text-[14px] font-bold tracking-[0.12em] uppercase text-asu-dark">
Next Event
</span> <svg class="w-5 h-5 text-asu-dark animate-bounce" viewBox="0 0 16 16" fill="none" aria-hidden="true"> <path d="M8 3v10M3.5 9l4.5 4.5L12.5 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </svg> </div> </section>`;
...
async function localizeExternalImage(url) {
  ...
  try {
    ...
    const targetDir = false ? join(process.cwd(), "public", "images", "events") : join(process.cwd(), "dist", "images", "events");
...
const upcoming = {"title":"No Upcoming Event","date":null,"startTime":null,"endTime":null,"location":null,"description":"There are no upcoming events announced at this time. Check back soon for updates from Asian Student Union.","imageUrl":null,"collab":null,"rsvpUrl":null,"tags":["Coming Soon"]};
const fallbackData = {
  upcoming,
};

const $$NextEventSection = createComponent(async ($$result, $$props, $$slots) => {
